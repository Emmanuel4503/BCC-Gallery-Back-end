...
const addImage = async (req, res) => {
  try {
    ...
    if (!req.files || req.files.length === 0) {
      return res
        .status(400)
        .json({ message: "No image files uploaded. Please select at least one image." });
    }
    if (!album || !type) {
    ...
    if (!["Normal", "Selected"].includes(type)) {
      return res
        .status(400)
        .json({ message: 'Invalid image type. Must be "Normal" or "Selected".' });
    }
    ...
    for (const file of req.files) {
      if (!allowedFormats.includes(file.mimetype)) {
        return res
          .status(400)
          .json({ message: `File ${file.originalname} is not a supported type (JPEG, PNG, WebP).` });
      }
      if (file.size > maxSize) {
        return res
          .status(400)
          .json({ message: `File ${file.originalname} exceeds 50MB limit.` });
      }
    }
    const sharedTimestamp = new Date();
    const imageDocs = req.files.map((file) => {
      const publicId = file.filename || `BCC Gallery Images/${file.originalname.split(".")[0]}`;
      return {
    ...
    console.log("Inserting images to MongoDB:", imageDocs);
...
const getImages = async (req, res) => {
  try {
    ...
    const latestImages = await Image.find(
      ...
      projection || "imageUrl thumbnailUrl album type reactions createdAt"
    );
    console.log("Fetched latest images:", latestImages.length);
...
const getSelectedImages = async (req, res) => {
  try {
    ...
    console.log("Fetched selected images:", selectedImages.length);
...
const getImagesByAlbum = async (req, res) => {
  try {
    ...
    console.log(`Fetched images for album ${album}:`, processedImages.length);
